...
export async function stopStream(appId: string): Promise<void> {
  await redisPublisher.publish(`events:${appId}`, JSON.stringify({ type: "abort-stream" }));
  await redisPublisher.del(`app:${appId}:stream-state`);
}

export async function waitForStreamToStop(appId: string, maxAttempts: number = 60): Promise<boolean> {
  ...
  if (hasStream === true) {
    return {
      async readableStream() {
        ...
        if (!stream) throw new Error("Failed to resume existing stream");
        return stream;
      },
      async response() {
        const resumableStream = await streamContext.resumeExistingStream(appId);
        if (!resumableStream) throw new Error("Failed to resume existing stream");
...
export async function setStream(appId: string, prompt: UIMessage, stream: any): Promise<StreamResponse> {
  ...
  if (!responseBody) {
    ...
    throw new Error("Error creating resumable stream: response body is undefined");
  }

  await redisPublisher.set(`app:${appId}:stream-state`, "running", { EX: 15 });

  const resumableStream = await streamContext.createNewResumableStream(appId, () => {
    return responseBody.pipeThrough(new TextDecoderStream()) as ReadableStream<string>;
  });
...
export async function handleStreamLifecycle(appId: string, event: "start" | "finish" | "error"): Promise<void> {
  switch (event) {
    case "start":
...
export async function sendMessageWithStreaming(
  ...
  const aiResponse = await AIService.sendMessage(agent, appId, mcpUrl, fs, message, {
    threadId: appId,
    resourceId: appId,
    maxSteps: 100,
    maxRetries: 0,
